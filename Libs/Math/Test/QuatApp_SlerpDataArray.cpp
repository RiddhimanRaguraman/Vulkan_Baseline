...
Quat QuatSourceA[] =
{
	Quat(0.5589f, 0.5322f, 0.5682f, 0.2854f),
...
TEST(QuatApp_SlerpDataArray, TestConfig::ALL)
{
	...
	for(unsigned int i = 0; i < numQuatA; i++)
	{
		QTEST(out2[i], QuatResultA[i]);
	}

	QuatApp::SlerpArray(out2, QuatSourceA, QuatTargetA, 1.0f, numQuatA);
	for(unsigned int i = 0; i < numQuatA; i++)
	{
		QTEST(out2[i], QuatTargetA[i]);
	}

	QuatApp::SlerpArray(out2, QuatSourceA, QuatTargetA, 0.0f, numQuatA);
	for(unsigned int i = 0; i < numQuatA; i++)
	{
		QTEST(out2[i], QuatSourceA[i]);
	}
